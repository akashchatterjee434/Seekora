...
const Login = () => {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#08090d] px-6 py-12 text-white">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(239,68,68,0.25),_transparent_36%),linear-gradient(135deg,_#08090d_20%,_#2b0d16_55%,_#10090d)]" />
      <div className="relative w-full max-w-md rounded-3xl border border-red-200/15 bg-black/35 p-8 shadow-2xl shadow-red-950/40 backdrop-blur-xl sm:p-10">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div>
            <label className="mb-2 block text-sm font-medium text-zinc-200" htmlFor="login-email">Email</label>
            <input
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-zinc-200" htmlFor="login-password">Password</label>
            <input
              className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none transition placeholder:text-zinc-600 focus:border-red-400 focus:ring-2 focus:ring-red-400/20"
              id="login-password"
              name="password"
        ...
        <p className="mt-8 text-center text-sm text-zinc-400">
          New to Seekora? <Link className="font-semibold text-red-300 hover:text-red-200" to="/register">Create an account</Link>
        </p>
      </div>
    </main>
  )
